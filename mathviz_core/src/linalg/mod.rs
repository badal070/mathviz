...
pub fn visualize(request: LinearTransformRequest) -> MathvizResult<LinearTransformResponse> {
    ...
    let x_axis = domain
        .x
        .as_ref()
        .ok_or_else(|| MathvizError::DomainViolation("linear transform requires x axis".to_string()))?;
    let y_axis = domain
        .y
        .as_ref()
        .ok_or_else(|| MathvizError::DomainViolation("linear transform requires y axis".to_string()))?;
    ...
    let (before, after) =
        transform::generate_transform_layers(&request.matrix, x_axis, y_axis, z_axis, density, prefix)?;
